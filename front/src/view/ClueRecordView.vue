      ...
      if (phone) {
        doGet("/api/clue/" + phone, {}).then(resp => {
          if (resp.data.code === 500) {
            ...
            return callback(); //验证通过了，直接调用一下callback()函数,不传值
          }
        })
      }
    },
      ...
      this.$refs.clueRefForm.validate( (isValid) =>  {
        if (isValid) {
          ...
          if (this.clueQuery.id > 0) {/*编辑*/
            doPost("/api/clue/edit", formData).then( (resp) => { //获取ajax异步请求后的结果
              if (resp.data.code === 200) {
                //编辑成功，提示一下
            ...
            doPost("/api/clue/add", formData).then( (resp) => { //获取ajax异步请求后的结果
              if (resp.data.code === 200) {
                //录入成功，提示一下
